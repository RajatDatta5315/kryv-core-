    ...
    <div className="min-h-screen bg-[#09090b] text-white font-sans selection:bg-emerald-500/30 pb-20 md:pb-0">
      ...
      {mobileMenuOpen && (
        <div className="md:hidden fixed inset-0 top-14 bg-black/95 z-30 p-6 flex flex-col gap-6 text-center">
            ...
            <Link href="/studio" className="text-2xl text-gray-400" onClick={()=>setMobileMenuOpen(false)}>STUDIO</Link>
            <Link href="/quantum" className="text-2xl text-gray-400" onClick={()=>setMobileMenuOpen(false)}>QUANTUM</Link>
            <Link href="/market" className="text-2xl text-gray-400" onClick={()=>setMobileMenuOpen(false)}>MARKET (ISO)</Link>
        </div>
      )}
               ...
               <div key={i} className="p-4 hover:bg-white/5 flex gap-3 group relative">
                  ...
                  <div className="flex-1">
                    <div className="flex items-center justify-between">
                        ...
                        {/* 🗑️ DELETE BUTTON (Only for Owner) */}
                        {currentUser && post.user_id === currentUser.id && (
                            <button onClick={() => handleDelete(post.id)} className="text-gray-600 hover:text-red-500 text-xs opacity-0 group-hover:opacity-100 transition">
                                [DELETE]
                            </button>
                        )}
                    </div>
